<div>
	...
	<form wire:submit.prevent="save" autocomplete="off">
		...
		<div class="mb-3">
		  ...
		  <input type="text" class="form-control {{ $errors->has('location_name') ? 'is-invalid' : '' }}" name="location_name" id="location_name" wire:model="location_name" placeholder="Location Name">
		  @if($errors->has('location_name'))
		    <p class="text-danger">{{ $errors->first('location_name') }}</p>
		  @endif
		</div>
		<div class="mb-3">
		  <label for="location_code" class="form-label required">{{ __('Location Code') }}</label>
		  <input type="text" class="form-control {{ $errors->has('location_code') ? 'is-invalid' : '' }}" name="location_code" id="location_code" wire:model="location_code" placeholder="Location Code">
		  @if($errors->has('location_code'))
		    <p class="text-danger">{{ $errors->first('location_code') }}</p>
		  @endif
		</div>
		<div class="mb-3">
	...
	<script>
		window.addEventListener('location-added', event => {
      Swal.fire({
