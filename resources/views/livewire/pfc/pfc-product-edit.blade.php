<div>
	...
	<form wire:submit.prevent="update" autocomplete="off">
		...
		<div class="mb-3">
		  ...
		  <input type="text" class="form-control {{ $errors->has('product.product_name') ? 'is-invalid' : '' }}" name="product_name" id="product_name" wire:model="product.product_name" placeholder="Product Name">
		  @if($errors->has('product.product_name'))
		    <p class="text-danger">{{ $errors->first('product.product_name') }}</p>
		  @endif
		</div>
		<div class="mb-3">
		  <label for="product_code" class="form-label required">{{ __('Product Code') }}</label>
		  <input type="text" class="form-control {{ $errors->has('product.product_code') ? 'is-invalid' : '' }}" name="product_code" id="product_code" wire:model="product.product_code" placeholder="Product Code">
		  @if($errors->has('product.product_code'))
		    <p class="text-danger">{{ $errors->first('product.product_code') }}</p>
		  @endif
		</div>
		<div class="mb-3">
		  <label for="product_description" class="form-label">{{ __('Product Description') }}</label>
		  <input type="text" class="form-control {{ $errors->has('product.product_description') ? 'is-invalid' : '' }}" name="product_description" id="product_description" wire:model="product.product_description" placeholder="Product Description">
		  @if($errors->has('product.product_description'))
		    <p class="text-danger">{{ $errors->first('product.product_description') }}</p>
		  @endif
		</div>
		<div class="mb-3">
