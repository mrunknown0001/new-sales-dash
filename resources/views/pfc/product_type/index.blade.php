...
@section('head-action')
	@if($gc->checkModuleAccess('product_type_add', 'PFC'))
		<a href="{{ route('pfc.product.type.add') }}" class="btn btn-link text-decoration-none">{{ __('ADD') }}</a>
	@endif
...
@section('content')
  <div class="row">
    <div class="col-md-12">
      <table id="product_types" class="table cell-border compact table-striped hover display nowrap" width="100%">
        <thead>
          <tr>
...
@section('scripts')
  ...
  <script>
    $(document).ready(function () {
      let customer_types = $('#product_types').DataTable({
        processing: true,
        serverSide: true,
        scrollX: true,
        columnDefs: [
    ...
    $(document).on('click', '#refresh', function(e) {
      var product_types = $('#product_types').DataTable();
    ...
    $(document).on('click', '#edit', function (e) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          var update_url = "{{ route('pfc.product.type.edit') }}"
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            ...
            url: "{{ route('pfc.product.type.delete') }}",
