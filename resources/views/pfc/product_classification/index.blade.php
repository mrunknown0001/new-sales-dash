...
@section('title')
	{{ __('PFC Product Classification') }}
@endsection

@section('head-action')
	@if($gc->checkModuleAccess('product_classification_add', 'PFC'))
		<a href="{{ route('pfc.product.classification.add') }}" class="btn btn-link text-decoration-none">{{ __('ADD') }}</a>
	@endif
...
@section('content')
  <div class="row">
    <div class="col-md-12">
      <table id="product_classifications" class="table cell-border compact table-striped hover display nowrap" width="100%">
        <thead>
          <tr>
            <th scope="col">{{ __('Product Classification') }}</th>
...
@section('scripts')
  ...
  <script>
    $(document).ready(function () {
      let customer_types = $('#product_classifications').DataTable({
        processing: true,
        serverSide: true,
        scrollX: true,
        columnDefs: [
        ...
        ajax: "{{ route('pfc.product.classification') }}",
        columns: [
    ...
    $(document).on('click', '#refresh', function(e) {
      var product_classifications = $('#product_classifications').DataTable();
      product_classifications.ajax.reload();
    });
    ...
    $(document).on('click', '#edit', function (e) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          var update_url = "{{ route('pfc.customer.type.edit') }}"
    ...
    $(document).on('click', '#delete', function (e) {
      ...
      var id = $(this).data('id');
      var name = $(this).data('name');
      var title = "Delete Customer Type?";
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          $.ajax({
            ...
            url: "{{ route('pfc.customer.type.delete') }}",
